...
//        String ttt = "state=http://172.17.0.13:80/wechat/get.do?{\"data\":\"\"}";
//        String substring = ttt.substring(ttt.indexOf("?")+1);
//        String url = ttt.substring(0, ttt.indexOf("?"));
//        Gson gson = new Gson();
//        MyData myData = gson.fromJson(substring, MyData.class);
//    }
    ...
    public String userInfo(@RequestParam("code") String code, @RequestParam("state") String state, ModelMap modelMap) {
        ...
        //解析state
        String substring = state.substring(state.indexOf("?")+1);
        String url = state.substring(0, state.indexOf("?"));
        Gson gson = new Gson();
        MyData myData = gson.fromJson(substring, MyData.class);
        ...
        try {
            param = "?nickname=" + URLEncoder.encode(nickname, "UTF-8") +
                    ...
                    "&data=" + URLEncoder.encode(myData.getData(), "UTF-8");
